				...
				<c:when  test="${!empty adminList}">
					<c:forEach items="${adminList}" var="i" varStatus="n">
						<tr>
							...
							<td>
								<a class="btn btn-info btn-sm" href="jsp/admin/adminUserEdit.do?id=${i.id}">修改</a>
								<a class="btn btn-danger btn-sm" href="jsp/admin/adminUserDelete.do?id=${i.id}" onclick="javascript:return confirm('确定要删除吗？');">删除</a>
							</td>
						</tr>
					</c:forEach>
				</c:when>
...
/*	$("#batDel").click(function(){
		...
		if(confirm("你确定要删除"+choices.length+"条用户吗？")){
			window.location="${basePath}jsp/admin/adminUserDeletes.do?ids="+ids;
		}
	})*/
	...
	$("#batDel").click(function(){
		...
		if(confirm("你确定要删除"+choices.length+"条用户吗？")){
			...
			window.location="${basePath}jsp/admin/adminUserDeletes.do?"+idParam;
		}
	})
