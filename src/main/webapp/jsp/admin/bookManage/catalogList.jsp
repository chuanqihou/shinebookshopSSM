				...
				<c:when test="${!empty catalogList}">
					<c:forEach items="${catalogList}" var="i" varStatus="n">
						<tr>
							...
							<td>${i.catalogName}</td>
							<td class="noClick">
								<a class="btn btn-danger btn-sm" href="jsp/admin/catalogDelete.do?id=${i.catalogId}" onclick="javascript:return confirm('确定要删除吗？');">删除</a>
							</td>
						</tr>
					</c:forEach>
				</c:when>
...
<script type="text/javascript">
	...
	$(".homePage").click(function(){
		window.location="${bsePath}jsp/admin/queryAllCatalog.do?page=1";
	})
	$(".prePage").click(function(){
		window.location="${basePath}jsp/admin/queryAllCatalog.do?page=${pageBean.prePage}";
	})
	$(".nextPage").click(function(){
		
		window.location="${basePath}jsp/admin/queryAllCatalog.do?page=${pageBean.nextPage}";
	})
	$(".lastPage").click(function(){
		
		window.location="${basePath}jsp/admin/queryAllCatalog.do?page=${pageBean.pageCount}";
	})
	//控制页面跳转范围
	$(".page-go").click(function(){
		...
		if(page > pageCount || page < 1 ){
			...
			window.location="${basePath}jsp/admin/queryAllCatalog.do?page="+page;
		}
	})
	...
	$("#batDel").click(function(){
		...
		if(confirm("你确定要删除"+choices.length+"条分类数据吗？")){
			var idParam = "";
			...
			window.location = "${basePath}jsp/admin/catalogDeletes.do?ids="+ids;
		}
	})
