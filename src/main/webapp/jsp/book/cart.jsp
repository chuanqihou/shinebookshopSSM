            ...
            <div class="content table-responsive">
                <table class="table">
                    <c:choose>
                        <c:when test="${!empty shopCart}">
                            ...
                            <c:forEach items="${shopCart.map}" var="i">
                                <tr class="row" id="pro-tr-${i.key}">
                                    ...
                                    <td><a class="btn btn-danger btn-sm" href="cartGoodsDel.do?bookId=${i.key}"
                                           onClick="return confirm('确定要删除此项了么？')">删除</a></td>
                                </tr>
                            </c:forEach>
                            <tr class="row">
                                <td colspan="2"><a href="cartGoodsDelAll.do" class="btn btn-danger">清空购物车</a></td>
                        ...
                        <c:otherwise>
                            <tr>
                                <td colspan="6" style="height: 200px; line-height: 100px; font-size: 24px;">
                                    购物车内暂时没有商品， <a style="font-size: 24px; color: red" href="jsp/book/index.jsp">去购物>></a>
                                </td>
                            </tr>
                        </c:otherwise>
                    </c:choose>
...
<script type="text/javascript">
    ...
    $("#tosettle").click(function () {
        $.get("loginStatus.do", function (data) {
            if (data.status == "y") {
    ...
    //验证码
    function reCode() {
        $("#imgCode").prop("src", "getLoginCode.do?action=" + new Date().getTime());
    }
</script>
