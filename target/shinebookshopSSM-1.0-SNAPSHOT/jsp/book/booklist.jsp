...
<body>
									...
									<c:choose>
										<c:when test="${!empty bookList}">
											<c:forEach items="${bookList}" var="i">
												<li class="col-md-3">
													<div class="list">
														<a href="bookdetail.do?bookId=${i.bookId}">
															<img class="img-responsive" src="${i.imgSrc}" />
														</a>
														<div class="proinfo">
															<h2>
																<a class="text-center" href="bookdetail.do?bookId=${i.bookId}">${i.bookName}</a>
															</h2>
															<p>
																<i>￥${i.price}</i>
	...
	<script type="text/javascript">
		...
		$(".homePage").click(function(){
			window.location="${bsePath}BookList.do?action=list&page=1";
		})
		$(".prePage").click(function(){
			window.location="${basePath}BookList.do?action=list&page=${pageBean.prePage}";
		})
		$(".nextPage").click(function(){

			window.location="${basePath}BookList.do?action=list&page=${pageBean.nextPage}";
		})
		$(".lastPage").click(function(){
			window.location="${basePath}BookList.do?action=list&page=${pageBean.pageCount}";
		})
		//控制页面跳转范围
		$(".page-go").click(function(){
			...
			if(page > pageCount || page < 1 ){
				...
				window.location="${basePath}BookList.do?action=list&page="+page;
			}
		})
	</script>
